...
(1) Game starts when the client conntects to the server and ends when the result is sent by the server.
(2) Board is send initially when the game ist started, contains the whole board with all nodes and their neighbors
(3) GameState Eeach round there is a Game state send by the server, it contains the current state of the game.
(4) Result< is sent when the game is finished. It contains the definition of the score fragments, the scores and their causes.
